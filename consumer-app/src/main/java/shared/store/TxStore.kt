...
/**
 * TxStore — ذخیره‌سازی محلی موجودی‌ها و تاریخچه تراکنش‌ها (SharedPreferences + JSON)
 * کیف‌ها: main, subsidy, emergency, cbdc
 */
class TxStore(context: Context) {
    ...
    data class Tx(
        ...
        val from: String,
        val to: String,
        val method: String, // QR | BLE
        val type: String,   // main | subsidy | emergency | cbdc
    ...
    fun getBalances(): Balances {
        ...
        return try {
            ...
            Balances(
                main = o.optLong("main", 10_000_000L),
    ...
    /** کاهش موجودی از کیف انتخابی */
    fun deduct(type: String, amount: Long): Balances {
        val b = getBalances()
        when (type) {
            "main" -> b.main -= amount
